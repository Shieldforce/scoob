...
GRANT ALL PRIVILEGES ON ${$mysql_db}.* TO 'scoob_user'@'%' WITH GRANT OPTION;

CREATE USER IF NOT EXISTS '${$mysql_user}'@'%.%.%.%' IDENTIFIED BY '${db_pass}';
GRANT ALL PRIVILEGES ON ${$mysql_db}.* TO 'scoob_user'@'%.%.%.%' WITH GRANT OPTION;

CREATE USER IF NOT EXISTS '${$mysql_user}'@'0.0.0.0' IDENTIFIED BY '${db_pass}';
GRANT ALL PRIVILEGES ON ${$mysql_db}.* TO 'scoob_user'@'0.0.0.0' WITH GRANT OPTION;

CREATE USER IF NOT EXISTS '${$mysql_user}'@'localhost' IDENTIFIED BY '${db_pass}';
GRANT ALL PRIVILEGES ON ${$mysql_db}.* TO 'scoob_user'@'localhost' WITH GRANT OPTION;
